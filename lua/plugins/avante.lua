return {
  {
    ...
    config = function()
      require("chatgpt").setup({
        -- Configure for Ollama with CodeLLaMA
      ...
      -- Cursor-based functionality (like AvanteAsk)
      vim.keymap.set("n", "<leader>aa", function()
        local input = vim.fn.input("Ask about current code: ")
        if input ~= "" then
          -- Include current file context
          local current_file = vim.fn.expand("%:p")
          local context = ""
          if current_file ~= "" then
            context = "Context: File " .. vim.fn.fnamemodify(current_file, ":t") .. "\n\n"
          end
          vim.cmd(":ChatGPT " .. context .. input)
        end
      end, { desc = "ChatGPT - Ask about current code" })
      
      -- Advanced cursor-based ask (with file context)
      vim.keymap.set("n", "<leader>aq", function()
        ...
        if input ~= "" then
          ...
          local context = "Here's the current file content:\n```" .. vim.bo.filetype .. "\n" .. content .. "\n```\n\nQuestion: " .. input
          vim.cmd(":ChatGPT " .. context)
        end
      end, { desc = "ChatGPT - Ask with full file context" })
      
      -- Quick cursor-based actions
      vim.keymap.set("n", "<leader>ax", ":ChatGPTRun explain_code<CR>", { desc = "ChatGPT - Explain code at cursor" })
      vim.keymap.set("n", "<leader>ay", ":ChatGPTRun fix_code<CR>", { desc = "ChatGPT - Fix code at cursor" })
      vim.keymap.set("n", "<leader>az", ":ChatGPTRun optimize_code<CR>", { desc = "ChatGPT - Optimize code at cursor" })
